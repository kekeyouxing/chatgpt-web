...
async function handleSelect({ uuid }: Correct.History) {
  if (isActive(uuid))
    return
...
function handleEdit({ uuid }: Correct.History, isEdit: boolean, event?: MouseEvent) {
...
function handleEnter({ uuid }: Correct.History, isEdit: boolean, event: KeyboardEvent) {
  event?.stopPropagation()
  if (event.key === 'Enter')
    correctStore.updateHistory(uuid, { isEdit })
}
